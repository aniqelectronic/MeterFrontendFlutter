...
class WeatherService {
  ...
  // Bentong district forecast.
  //
  // Ds059 = Bentong district
  // Tn072 = Bentong town
  //
  // We use Ds059 only so every date appears once.
  static const String _bentongLocationId = 'Ds059';

  // Changed to v2 so the old duplicated cache is ignored.
  static const String _cacheJsonKey =
      'weather_bentong_ds059_cache_json_v2';

  static const String _cacheTimeKey =
      'weather_bentong_ds059_cache_time_v2';
  ...
  Future<WeatherResult> getBentongForecast({
  ...
  Future<WeatherResult> _fetchFromNetwork() async {
    // Ask the API for the exact Bentong district location ID.
    final uri = Uri.parse(_baseUrl).replace(
      queryParameters: {
        'contains':
            '$_bentongLocationId@location__location_id',
      },
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final parsedForecasts = rawList
            ...
            .where(
              (item) =>
                  item.locationId == _bentongLocationId,
            )
            .toList();

        // Extra protection against duplicate dates.
        ...
        if (forecasts.isEmpty) {
          throw const FormatException(
            'No Bentong district weather records returned.',
          );
        }
  ...
  Future<WeatherResult?>
      _readPersistentCache() async {
    try {
      final prefs =
          await SharedPreferences.getInstance();

      final jsonText =
          prefs.getString(_cacheJsonKey);

      final savedAtText =
          prefs.getString(_cacheTimeKey);
      ...
      // Prevent duplicate dates from old or malformed cache.
      final Map<String, WeatherForecast> uniqueByDate = {};
